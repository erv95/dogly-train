...
import { CaretakerProfile, CoinTransaction, User } from '../../src/types';
...
export default function CaretakerCoinsScreen() {
  ...
  const boostTime = getBoostTimeRemaining(caretaker?.boostedUntil);
  ...
  const handleBoost = () => {
    if (!firebaseUser || !caretaker) return;

    if ((caretaker.coinBalance ?? 0) < BOOST_COST) {
        ...
        { text: t('common.cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            try {
              ...
              Alert.alert(t('common.ok'), t('caretaker.boostActive'));
              loadTransactions();
            } catch (error: any) {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      >
        <Text style={styles.title}>{t('caretaker.coins')}</Text>

        <Card style={styles.balanceCard}>
          <Ionicons name="wallet" size={32} color={colors.primary} />
          <Text style={styles.balanceLabel}>{t('coins.balance')}</Text>
          <Text style={styles.balanceValue}>{caretaker?.coinBalance ?? 0}</Text>
        </Card>
        ...
        <View style={styles.actions}>
          ...
          {!isBoosted && (
            <Button
              ...
              variant="outline"
              size="lg"
              disabled={(caretaker?.coinBalance ?? 0) < BOOST_COST}
            />
          )}
        </View>
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: fontSize.xxl, fontWeight: '700', color: colors.text, marginBottom: spacing.lg, textAlign: 'center' },
  balanceCard: { alignItems: 'center', paddingVertical: spacing.xl, marginBottom: spacing.md },
  balanceLabel: { fontSize: fontSize.md, color: colors.textSecondary, marginTop: spacing.sm },
  balanceValue: { fontSize: 48, fontWeight: '800', color: colors.primary, marginTop: spacing.xs },
  boostBanner: {
  ...
  txType: { fontSize: fontSize.sm, fontWeight: '600', color: colors.text },
